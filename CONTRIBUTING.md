...
- When developing or publishing the extension locally, you need to create a `.env` file at the top level directory of the front end project (where `package.json` lives). In lieu of the `.env` file you can set actual environment variables. The contents of the `.env` file are
